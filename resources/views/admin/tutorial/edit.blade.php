...
@section('judul')
    Edit Prosedur Sertifikasi | Admin LSP
@endsection
...
@section('isi')
    ...
    <div class="page-header">
        <h3>
            <i class="fas fa-edit"></i> Edit Prosedur Sertifikasi
        </h3>
        ...
        <div class="card-body">
            <form action="{{ route('tutorial.update', $tutorial->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <textarea name="konten" class="summernote @error('konten') is-invalid @enderror">{{ old('konten', $tutorial->konten) }}</textarea>
                    @error('konten')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="url" name="tautan" class="form-control @error('tautan') is-invalid @enderror" value="{{ old('tautan', $tutorial->tautan) }}" placeholder="https://example.com">
                    @error('tautan')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="number" name="urutan" class="form-control @error('urutan') is-invalid @enderror" value="{{ old('urutan', $tutorial->urutan) }}" placeholder="Urutan tampilan (1, 2, 3, ...)">
                    @error('urutan')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
