...
@section('judul')
    FR.AK.04 | ASESI {{ $site_setting->title ?? 'Lembaga Sertifikasi Profesi' }}
@endsection
                                    ...
                                    <table id="order-listing" class="table dataTable no-footer" role="grid">
                                         <thead>
                                             <tr class="bg-danger text-white" role="row">
                                                 ...
                                                 <th class="sorting" style="width: 150px;">Status FR.AK.04</th>
                                             </tr>
                                         </thead>
                                         <tbody>
                                             @forelse ($registrations as $reg)
                                                 @php
                                                     $frAk04 = \App\Models\FrAk04::where('data_register_id', $reg->id)->first();
                                                 @endphp
                                                 <tr role="row" class="odd">
                                                     <td class="font-weight-bold">{{ $loop->iteration }}</td>
                                                     <td class="text-right">
                                                         <a href="{{ route('frak04.create', $reg->id) }}" class="btn btn-dark btn-sm font-weight-bold">
